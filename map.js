...
typeSelect.onchange = function () {
    ...
    EtkilesimiEkle();
    map.un('click', mapEditdeTiklandiginda);
};
...
function CizimBittiginde(evt) {
  ...
  SonCizimSilinsinMi=true;
  ModalAc("","","","",wktString, feature);

}

function SonCizimiSil(){
  //cizim yapilan katmandan ozellikleri al
  var nesneler = kaynak.getFeatures();
  //sonuncu olarak ekleneni sec
  var SonuncuFeature = nesneler[nesneler.length - 1];
  //kaynaktan souncu olani sil
  CizimiSil(SonuncuFeature);    
}

function CizimiSil(Feature){
   //kaynaktan Feature sil
  kaynak.removeFeature(Feature);    
}
